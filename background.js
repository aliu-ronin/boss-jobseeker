...
const EVAL_PROMPT = `你是一个求职顾问。判断这条消息是否值得回复。
...
## ⚠️ 优先判断：对方是否已婉拒/不匹配
先分析"对方消息"，如果对方表达了以下任一意思，直接判定 REJECT，不再计分：
- 明确表示不匹配、不合适、不符合要求、经验/能力/方向有差距
- 委婉拒绝：很遗憾、暂不考虑、岗位已招满、已找到合适人选
- 通知淘汰：未通过筛选、简历未通过、不能与您共事
- 结束对话：祝您早日找到心仪工作、祝顺利等告别语（在拒绝语境下）
→ 命中则直接输出 {"score": 0, "reason": "对方婉拒:具体原因", "action": "REJECT", "detail": "对方已明确表示不匹配/拒绝"}

## 计分规则（仅在未被婉拒时使用）
无基础分，薪资分就是起点，然后叠加：
1. **薪资分**（见上方计算规则，就是基础分）
2. **地点分**（见上方计算规则）
3. **AI/智能体/Agent 相关岗位或话题** = +10（高度匹配方向）
4. **未涉及 AI/智能体/Agent** = -5（方向不匹配）
5. **猎头/HR/招聘主动联系** = +5（有机会就聊）
6. **垃圾/诈骗/推销**（副业、兼职、月入X万、招商加盟、退税、刷单、理财、保险代理、微商、传销、个人所得税）= -1000
7. **黑名单行业**（博彩/P2P/资金盘/网络赌博/棋牌）= -1000
8. **信息不足**（打招呼、没提薪资、没提岗位）= -50
9. **知名公司**（BAT、字节、华为、美团、腾讯等）= +10
10. **上市公司** = +5
11. **明确996/大小周** = -20
12. **外包/外派/驻场/人力外包** = -100
...
async function llmEvaluate(config, jobInfo) {
  ...
  const prompt = fillPrompt(EVAL_PROMPT, {
...
async function handleGetConfig() {
  ...
  return {
    ...
    filters: { salary_baseline_k: c.filters?.salary_baseline_k || 500, cities: c.filters?.cities || [], blacklist_industries: c.filters?.blacklist_industries || [] },
